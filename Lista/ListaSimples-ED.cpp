...
int Verifica_letra( Lista *l){
	int i, aux, tam_descricao;
	for(i=0;i<l->tamanho; i++){
	    tam_descricao= srtlen(l->lista[i].descricao);
	    if(l->lista[i].descricao[tam_descricao]== "a"||"e"||"i"||"o"||"u"){
	    	aux=1;
		}
	}
	if(aux>0)
...
int Compara_lista(Lista *l, Lista2 *l){
	int = cont1=0, cont2=0;
...
main(){
	...
	int codigo_buscar;
	fazListaVazia(&lista); // Inicializa as variáveis de controle da estrutura
	...
	printf("\n Ocorrencia de vogal na ultima letra da descricao: %d\n", Verifica_letra(&lista));
